...
class ProcessedPatientEntry {
  /// Returns a new [ProcessedPatientEntry] instance.
  ProcessedPatientEntry({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ProcessedPatientEntry &&
  ...
  /// Returns a new [ProcessedPatientEntry] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ProcessedPatientEntry? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ProcessedPatientEntry[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ProcessedPatientEntry[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ProcessedPatientEntry(
  ...
  static List<ProcessedPatientEntry> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ProcessedPatientEntry>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ProcessedPatientEntry.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ProcessedPatientEntry> mapFromJson(dynamic json) {
    final map = <String, ProcessedPatientEntry>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ProcessedPatientEntry.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ProcessedPatientEntry-objects as value to a dart map
  static Map<String, List<ProcessedPatientEntry>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ProcessedPatientEntry>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ProcessedPatientEntry.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
