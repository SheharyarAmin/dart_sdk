//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';


/// tests for ConsultantsApi
void main() {
  // final instance = ConsultantsApi();

  group('tests for ConsultantsApi', () {
    // Change Active Status
    //
    //Future<Map<String, String>> changeActiveStatusApiV1ConsultantsConsultantIdChangeActiveStatusPut(String consultantId) async
    test('test changeActiveStatusApiV1ConsultantsConsultantIdChangeActiveStatusPut', () async {
      // TODO
    });

    // Change Deleted Status
    //
    //Future<Map<String, String>> changeDeletedStatusApiV1ConsultantsConsultantIdChangeDeletedStatusPut(String consultantId) async
    test('test changeDeletedStatusApiV1ConsultantsConsultantIdChangeDeletedStatusPut', () async {
      // TODO
    });

    // Create New Consultant
    //
    //Future<Consultant> createNewConsultantApiV1ConsultantsPost(ConsultantCreate consultantCreate) async
    test('test createNewConsultantApiV1ConsultantsPost', () async {
      // TODO
    });

    // Delete Existing Consultant
    //
    //Future<Consultant> deleteExistingConsultantApiV1ConsultantsConsultantIdDelete(String consultantId) async
    test('test deleteExistingConsultantApiV1ConsultantsConsultantIdDelete', () async {
      // TODO
    });

    // Get Consultant Invoice
    //
    //Future getConsultantInvoiceApiV1ConsultantsConsultantInvoicePost(Map<String, String> requestBody) async
    test('test getConsultantInvoiceApiV1ConsultantsConsultantInvoicePost', () async {
      // TODO
    });

    // Read Consultant
    //
    //Future<Map<String, Consultant>> readConsultantApiV1ConsultantsConsultantIdGet(String consultantId) async
    test('test readConsultantApiV1ConsultantsConsultantIdGet', () async {
      // TODO
    });

    // Read Consultants
    //
    //Future<Map<String, Consultant>> readConsultantsApiV1ConsultantsGet() async
    test('test readConsultantsApiV1ConsultantsGet', () async {
      // TODO
    });

    // Update Existing Consultant
    //
    //Future<Consultant> updateExistingConsultantApiV1ConsultantsConsultantIdPut(String consultantId, ConsultantUpdate consultantUpdate) async
    test('test updateExistingConsultantApiV1ConsultantsConsultantIdPut', () async {
      // TODO
    });

  });
}
